...
____ic ssize_t gpio_read( struct file* F, char *buf, size_t count, loff_t *f_pos )
{
	char buffer[10];
...
static ssize_t gpio_write( struct file* F, const char *buf, size_t count, loff_t *f_pos )
{
	...
	switch( buffer[0] )
	{
		case '0':
			...
			gpio_set_value(GPIO_NUMBER16, 0);
                        break;

		case '1':
			gpio_set_value(GPIO_NUMBER, 0);
			gpio_set_value(GPIO_NUMBER16, 1);
			break;
		case '2':
...
static int init_gpio(void)
{
	...
	*/
	...
	if(0<gpio_direction_output(GPIO_NUMBER16, 1))
		return -1;
        //set pin15 to output
