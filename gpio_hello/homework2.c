...
void my_timer_OFF( unsigned long data );
/*------------------------timer----------------*/
void my_timer_ON( unsigned long data )
{
     /* do your timer stuff here */
     gpio_set_value(GPIO_NUMBER, 1);
     setup_timer(&my_timer, my_timer_OFF, 0);
...
void my_timer_OFF( unsigned long data )
{
     ...
     setup_timer(&my_timer, my_timer_ON, 0);
     mod_timer(&my_timer, jiffies + msecs_to_jiffies(200));
}

int init_module_ON(void)
{
...
/*-------------------------slow-------------------------*/
void my_timer_slow_OFF( unsigned long data );
void my_timer_slow_ON( unsigned long data )
{
     /* do your timer stuff here */
     gpio_set_value(GPIO_NUMBER, 1);
     setup_timer(&my_timer, my_timer_slow_OFF, 0);
...
void my_timer_slow_OFF( unsigned long data )
{
     /* do your timer stuff here */
     gpio_set_value(GPIO_NUMBER, 0);
     setup_timer(&my_timer, my_timer_slow_ON, 0);
     mod_timer(&my_timer, jiffies + msecs_to_jiffies(1000));
}
int init_module_slow_ON(void)
{
  /* setup your timer to call my_timer_callback */
  setup_timer(&my_timer, my_timer_ON, 0);
  /* setup timer interval to 200 msecs */
